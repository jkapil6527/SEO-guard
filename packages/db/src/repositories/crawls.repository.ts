...
export class CrawlsRepository {
  ...
  findById(id: string): Promise<CrawlRow | null> {
    return this.db.queryOne<CrawlRow>(`SELECT ${COLS} FROM crawls WHERE id = $1`, [id]);
  }
  ...
  findReportById(id: string): Promise<CrawlReportRow | null> {
    return this.db.queryOne<CrawlReportRow>(
      `SELECT c.id, c.website_id AS "websiteId", c.status, c.trigger, c.mode, c.scope,
  ...
  findStuckFinalizing(staleBefore: Date): Promise<CrawlRow[]> {
    return this.db.query<CrawlRow>(
      `SELECT ${COLS} FROM crawls
  ...
  listByWebsite(websiteId: string, limit: number, beforeCreatedAt?: Date): Promise<CrawlRow[]> {
    if (beforeCreatedAt) {
      return this.db.query<CrawlRow>(
        `SELECT ${COLS} FROM crawls WHERE website_id = $1 AND created_at < $2
         ORDER BY created_at DESC LIMIT $3`,
        [websiteId, beforeCreatedAt, limit],
      );
    }
    return this.db.query<CrawlRow>(
      `SELECT ${COLS} FROM crawls WHERE website_id = $1 ORDER BY created_at DESC LIMIT $2`,
      [websiteId, limit],
    );
  }
  ...
  findActiveForScope(websiteId: string, sitemapGroupId?: string | null): Promise<CrawlRow | null> {
    if (sitemapGroupId) {
      return this.db.queryOne<CrawlRow>(
        `SELECT ${COLS} FROM crawls
          WHERE website_id = $1 AND status = ANY($2)
            AND (sitemap_group_id = $3 OR sitemap_group_id IS NULL)
          ORDER BY created_at DESC LIMIT 1`,
        [websiteId, ACTIVE_STATUSES, sitemapGroupId],
      );
    }
    return this.db.queryOne<CrawlRow>(
      `SELECT ${COLS} FROM crawls WHERE website_id = $1 AND status = ANY($2)
       ORDER BY created_at DESC LIMIT 1`,
      [websiteId, ACTIVE_STATUSES],
    );
  }
    ...
    if (sitemapGroupId) {
      return this.db.queryOne<CrawlRow>(
        `SELECT ${COLS} FROM crawls
          ...
          ORDER BY created_at DESC LIMIT 1`,
        [websiteId, beforeCrawlId, sitemapGroupId],
      );
    }
    return this.db.queryOne<CrawlRow>(
      `SELECT ${COLS} FROM crawls
  ...
  listAll(limit: number, beforeCreatedAt?: Date, projectId?: string): Promise<CrawlReportRow[]> {
    ...
    return this.db.query<CrawlReportRow>(
      `SELECT c.id, c.website_id AS "websiteId", c.status, c.trigger, c.mode, c.scope,
              c.target_url AS "targetUrl", c.sitemap_group_id AS "sitemapGroupId",
              c.rule_pack_version AS "rulePackVersion", c.stats,
              c.error, c.started_at AS "startedAt", c.finished_at AS "finishedAt",
  ...
  async create(input: {
    ...
    const row = await this.db.queryOne<CrawlRow>(
      `INSERT INTO crawls (website_id, trigger, mode, scope, target_url, sitemap_group_id,
  ...
  async setStatus(id: string, status: string, patch?: { error?: string }): Promise<void> {
    await this.db.query(
      `UPDATE crawls SET
  ...
  async updateStats(id: string, stats: Record<string, number>): Promise<void> {
