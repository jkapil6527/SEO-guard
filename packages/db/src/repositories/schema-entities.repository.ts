...
export class SchemaEntitiesRepository {
  ...
  listByCrawl(
    ...
    return this.db.query<SchemaEntityRow>(
      `SELECT ${COLS}, p.url FROM schema_entities e JOIN pages p ON p.id = e.page_id
       WHERE ${where} ORDER BY e.created_at, e.id LIMIT $${params.length}`,
      params,
    );
  }
  ...
  listByPage(crawlId: string, pageId: string): Promise<SchemaEntityRow[]> {
    return this.db.query<SchemaEntityRow>(
      `SELECT ${COLS}, p.url FROM schema_entities e JOIN pages p ON p.id = e.page_id
       WHERE e.crawl_id = $1 AND e.page_id = $2 ORDER BY e.schema_type`,
      [crawlId, pageId],
    );
  }

  typeFrequency(crawlId: string): Promise<Array<{ schemaType: string; count: number }>> {
    return this.db.query(
      `SELECT schema_type AS "schemaType", count(*)::int AS count
       FROM schema_entities WHERE crawl_id = $1 GROUP BY schema_type ORDER BY count DESC`,
      [crawlId],
    );
  }

  statusCounts(crawlId: string): Promise<Array<{ status: string; count: number }>> {
    return this.db.query(
      `SELECT status, count(*)::int AS count FROM schema_entities
       WHERE crawl_id = $1 GROUP BY status`,
      [crawlId],
    );
  }

  /** Coverage: total entities, and how many distinct pages carry any schema. */
  async coverage(crawlId: string): Promise<{
    ...
    const row = await this.db.queryOne<{
      ...
      `SELECT count(*)::int AS "totalEntities",
       ...
       FROM schema_entities WHERE crawl_id = $1`,
      [crawlId],
    );
  ...
  richResultSummary(
    crawlId: string,
  ): Promise<Array<{ profile: string; status: string; count: number }>> {
    return this.db.query(
      `SELECT rr ->> 'profile' AS profile, rr ->> 'status' AS status, count(*)::int AS count
  ...
  async pageIdsWithSchema(crawlId: string): Promise<string[]> {
    const rows = await this.db.query<{ pageId: string }>(
      `SELECT DISTINCT page_id AS "pageId" FROM schema_entities WHERE crawl_id = $1`,
      [crawlId],
    );
   ...
   * is skipped.
   */
  loadSummariesForPages(
    crawlId: string,
    pageIds: string[],
    maxPropertyBytes = 262144,
  ): Promise<SchemaSummaryRow[]> {
    if (pageIds.length === 0) return Promise.resolve([]);
    return this.db.query<SchemaSummaryRow>(
      `SELECT e.page_id AS "pageId", p.url, e.schema_type AS "schemaType", e.format, e.status,
       ...
       WHERE e.crawl_id = $1 AND e.page_id = ANY($2)`,
      [crawlId, pageIds, maxPropertyBytes],
    );
  }

  /** Copies a previous snapshot's schema entities onto a carried-forward snapshot. */
  async copyForCarryForward(
    fromCrawlId: string,
