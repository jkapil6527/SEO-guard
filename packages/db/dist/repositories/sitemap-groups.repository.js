...
exports.SitemapGroupsRepository = void 0;
...
class SitemapGroupsRepository {
    db;
    constructor(db) {
        this.db = db;
    }
    findById(id) {
        return this.db.queryOne(`SELECT ${COLS} FROM sitemap_groups g WHERE g.id = $1`, [id]);
    }
    listByWebsite(websiteId) {
        return this.db.query(`SELECT ${COLS} FROM sitemap_groups g WHERE g.website_id = $1 ORDER BY g.name`, [websiteId]);
    }
    /**
    ...
    listSummariesByProject(projectId) {
        return this.db.query(`SELECT ${COLS},
        ...
        ORDER BY w.name, g.name`, [projectId]);
    }
    /** 30-day score history for a group's card sparkline. */
    trend(groupId, days = 30) {
        return this.db.query(`SELECT day, seo_score AS "seoScore" FROM trend_daily_group
        WHERE sitemap_group_id = $1 AND day >= current_date - $2::int
        ORDER BY day`, [groupId, days]);
    }
    async create(input) {
        const row = await this.db.queryOne(`INSERT INTO sitemap_groups (website_id, name, slug, sitemap_url, url_source_id, created_by)
       VALUES ($1, $2, $3, $4, $5, $6)
    ...
    async update(id, patch) {
        return this.db.queryOne(`UPDATE sitemap_groups SET
       ...
       RETURNING id, website_id AS "websiteId", name, slug, sitemap_url AS "sitemapUrl",
                 url_source_id AS "urlSourceId", is_active AS "isActive", settings,
                 created_at AS "createdAt", updated_at AS "updatedAt"`, [id, patch.name ?? null, patch.sitemapUrl ?? null, patch.isActive ?? null]);
    }
    async remove(id) {
    ...
    async linkPages(groupId, pages) {
        ...
        for (let i = 0; i < pages.length; i += CHUNK) {
            ...
            await this.db.query(`INSERT INTO page_sitemap_groups (sitemap_group_id, page_id, lastmod)
         VALUES ${values.join(', ')}
         ON CONFLICT (page_id, sitemap_group_id)
         DO UPDATE SET lastmod = EXCLUDED.lastmod`, params);
        }
    }
}
exports.SitemapGroupsRepository = SitemapGroupsRepository;
//# sourceMappingURL=sitemap-groups.repository.js.map