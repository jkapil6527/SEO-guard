 ...
 */
import type { PropertySpec, VocabPack, VocabTypeSpec } from '../types';

type CompactProps = Record<string, string[]>;

function props(compact: CompactProps): Record<string, PropertySpec> {
  const out: Record<string, PropertySpec> = {};
  for (const [name, expected] of Object.entries(compact)) out[name] = { expected };
  return out;
}

function type(parents: string[], compact: CompactProps): VocabTypeSpec {
  return { parents, properties: props(compact) };
}

const types: Record<string, VocabTypeSpec> = {
  Thing: type([], {
    name: ['Text'],
    alternateName: ['Text'],
    description: ['Text'],
...
export const VOCAB_PACK: VocabPack = {
  version: '1.0.0',
  types,
  enumerations,
