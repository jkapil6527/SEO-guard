...
function expandIPv6(ip) {
    ...
    if (text.includes('.')) {
        ...
        if (node_net_1.default.isIP(v4) !== 4) {
            return null;
        }
    ...
    if (halves.length === 2) {
        ...
        raw = [...head, ...Array(missing).fill('0'), ...tail];
    }
    else {
        raw = head;
    }
...
class SafeFetcher {
    ...
    constructor(options) {
        ...
        this.allowPrivate = new Set((options.allowPrivateTargets ?? []).map((target) => target.toLowerCase()));
    }
    async fetch(url, opts = {}) {
        ...
        for (;;) {
            ...
            if (Date.now() >= deadline) {
                return this.failure(canonicalUrl, redirectChain, started, 'TIMEOUT', `timed out after ${timeoutMs}ms`);
            }
            ...
            if (REDIRECT_STATUSES.has(exchange.status) && location !== undefined) {
                ...
                if (redirectChain.length > maxRedirects) {
                    return this.failure(canonicalUrl, redirectChain, started, 'TOO_MANY_REDIRECTS', `exceeded ${maxRedirects} redirects`);
                }
                let next;
                try {
                    ...
                    return this.failure(canonicalUrl, redirectChain, started, 'CONNECTION_ERROR', `invalid redirect Location: ${location}`);
                }
    ...
    async guard(urlStr) {
        ...
        const bareHost = url.hostname.startsWith('[') && url.hostname.endsWith(']')
            ...
            : url.hostname;
        const allowListed = this.allowPrivate.has(`${bareHost.toLowerCase()}:${port}`) ||
        ...
        const literal = node_net_1.default.isIP(bareHost);
        if (literal !== 0) {
            ...
            return { ok: true, url, bareHost, port, address: bareHost, family: literal };
        }
        let records;
        try {
            records = await node_dns_1.default.promises.lookup(bareHost, { all: true });
        }
        catch (err) {
    ...
    performRequest(guard, method, opts, maxBodyBytes, deadline) {
        return new Promise((resolve) => {
            ...
            const pinnedLookup = (_hostname, lookupOpts, callback) => {
            ...
            if (isTls) {
                ...
                requestOptions.servername = node_net_1.default.isIP(bareHost) === 0 ? bareHost : '';
            }
            const transport = isTls ? node_https_1.default : node_http_1.default;
            const req = transport.request(requestOptions, (res) => {
                const status = res.statusCode ?? 0;
                const responseHeaders = flattenHeaders(res.headers);
                const skipBody = method === 'HEAD' ||
                    status === 304 ||
                ...
                let source = res;
                const decompressor = contentEncoding === 'gzip' || contentEncoding === 'x-gzip'
                    ? node_zlib_1.default.createGunzip()
                    : contentEncoding === 'deflate'
                        ? node_zlib_1.default.createInflate()
                        : contentEncoding === 'br'
                            ? node_zlib_1.default.createBrotliDecompress()
                            : null;
                ...
                source.on('end', () => {
                    finish({ kind: 'response', status, headers: responseHeaders, body: Buffer.concat(chunks) }, false);
                });
                source.on('error', (err) => {
            ...
            req.on('error', (err) => {
                const code = err.code === 'ETIMEDOUT' || err.code === 'ECONNABORTED' ? 'TIMEOUT' : 'CONNECTION_ERROR';
                finish({ kind: 'error', code, message: err.message }, true);
            });
            req.end();
        });
    }
    failure(finalUrl, redirectChain, started, code, message) {
        return {
            finalUrl,
            status: 0,
            ok: false,
...
exports.SafeFetcher = SafeFetcher;
...
//# sourceMappingURL=safe-fetcher.js.map