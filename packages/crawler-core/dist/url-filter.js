...
function globToRegex(glob) {
    ...
    const pattern = glob.startsWith('/') || glob.startsWith('*') || glob.startsWith('?') ? glob : `/${glob}`;
    let out = '';
...
class UrlFilter {
    ...
    block;
    constructor(options) {
        this.originDomain = (0, url_1.registrableDomain)(new URL(options.origin).hostname);
    ...
    isInScope(url) {
        ...
        if ((0, url_1.registrableDomain)(parsed.hostname) !== this.originDomain) {
            return false;
        }
