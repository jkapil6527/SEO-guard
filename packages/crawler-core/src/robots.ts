...
interface RobotsRule {
  allow: boolean;
  /** Raw pattern; its length drives longest-match precedence. */
  pattern: string;
  regex: RegExp;
}

interface RobotsGroup {
  /** Lowercased user-agent tokens; a group can list several. */
  userAgents: string[];
  rules: RobotsRule[];
...
export function parseRobots(txt: string): RobotsRules {
  ...
  const matchGroups = (userAgent: string): RobotsGroup[] => {
    ...
    return groups.filter((group) =>
      group.userAgents.some(
        (token) => (token === '*' ? 0 : ua.includes(token) ? token.length : -1) === best,
      ),
    );
  };
