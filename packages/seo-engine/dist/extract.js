...
function extractMetaByName($, name) {
...
function extractRobots($, headers) {
    const raw = extractMetaByName($, 'robots');
    const metaTokens = (0, util_1.robotsTokens)(raw);
    const headerRaw = header(headers, 'x-robots-tag');
    const headerTokens = (0, util_1.robotsTokens)(headerRaw);
    return {
        raw,
...
function extractCanonicals($, base) {
    const out = [];
    $('link[rel~="canonical" i]').each((_i, el) => {
        ...
        const resolved = (0, util_1.resolveUrl)(href, base);
        if (resolved !== null) {
            out.push(resolved);
        }
    });
...
function selectorOf($, el) {
    ...
    while (current && depth < 6) {
        ...
        if (sameTagSiblings.length > 1) {
            const index = sameTagSiblings.index(node) + 1;
...
function effectiveImageSrc($el) {
    ...
    const fromSrcset = firstFromSrcset($el.attr('data-srcset')) ?? firstFromSrcset($el.attr('srcset'));
...
function extractImages($, base) {
    ...
    $('img').each((_i, el) => {
        ...
        const resolved = rawSrc === undefined ? null : (0, util_1.resolveUrl)(rawSrc, base);
...
function extractLinks($, base, pageDomain) {
    const out = [];
    $('a[href]').each((_i, el) => {
        const $el = $(el);
        ...
        const href = (0, util_1.resolveUrl)(rawHref, base);
        if (href === null) {
            return;
        }
        const rel = $el.attr('rel') ?? null;
        const relTokens = rel === null ? [] : rel.toLowerCase().split(/\s+/);
        const linkDomain = (0, util_1.urlRegistrableDomain)(href);
        out.push({
            href,
            text: $el.text().trim(),
            rel,
...
function extractMixedContent($, base, isHttps) {
    if (!isHttps) {
        return [];
    }
    const out = new Set();
    for (const { selector, attr } of SUBRESOURCE_SELECTORS) {
        $(selector).each((_i, el) => {
            ...
            const resolved = (0, util_1.resolveUrl)(value, base);
...
function extractArtifacts(html, ctx) {
    const $ = (0, cheerio_1.load)(html ?? '');
    const base = ctx.finalUrl;
    const pageDomain = (0, util_1.urlRegistrableDomain)(base);
    let isHttps = false;
    try {
    ...
    const htmlLang = htmlLangAttr !== undefined && htmlLangAttr.trim().length > 0 ? htmlLangAttr.trim() : null;
    return {
        url: ctx.url,
        ...
        titleHash: (0, util_1.normalizedHash)(title),
        descriptionHash: (0, util_1.normalizedHash)(metaDescription),
        h1Hash: (0, util_1.normalizedHash)(h1Text),
        h1Text,
    };
}
