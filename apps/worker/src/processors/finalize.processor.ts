...
import type { CrawlChangeInput, SchemaSummaryRow } from '@seo-guardian/db';
import { CHECK_IDS, MAX_REDIRECT_HOPS, getCatalogCheck } from '@seo-guardian/seo-engine';
import { diffPageSchema } from '@seo-guardian/schema-engine';
import type { SchemaEntitySummary } from '@seo-guardian/schema-engine';
import { CrawlStatus, IssueSeverity, QUEUES } from '@seo-guardian/shared';
...
function severityOf(id: string): IssueSeverity {
  return getCatalogCheck(id)?.defaultSeverity ?? IssueSeverity.Medium;
}
...
@Processor(QUEUES.CRAWL_FINALIZE, {
  concurrency: 2,
...
export class FinalizeProcessor extends WorkerHost {
  ...
  override async process(job: Job<FinalizeJobData>, token?: string): Promise<unknown> {
    const { crawlId } = job.data;
    const crawl = await this.crawls.findById(crawlId);
    if (!crawl) return { skipped: 'crawl gone' };
    ...
    const waits = (job.data as { _waits?: number })._waits ?? 0;
    if ((await this.state.getLinkBatches(crawlId)) > 0 && waits < MAX_LINK_WAITS) {
      job.data = { ...job.data, _waits: waits + 1 } as FinalizeJobData;
      await job.moveToDelayed(Date.now() + 1000, token);
      throw new DelayedError();
    }

    await this.crawls.setStatus(crawlId, CrawlStatus.Finalizing);

    // Cross-page duplicate issues (title/description/h1).
    const dup = [
      { hash: 'titleHash', sample: 'title', id: CHECK_IDS.DUPLICATE_TITLE, field: 'title' },
      {
      ...
      { hash: 'h1Hash', sample: 'h1Text', id: CHECK_IDS.DUPLICATE_H1, field: 'h1' },
    ] as const;
    ...
    const brokenLinkIssues = await this.issues.insertBrokenLinkIssues({
      crawlId,
      internalCheckId: CHECK_IDS.LINK_INTERNAL_BROKEN,
      internalSeverity: severityOf(CHECK_IDS.LINK_INTERNAL_BROKEN),
      externalCheckId: CHECK_IDS.LINK_EXTERNAL_BROKEN,
      externalSeverity: severityOf(CHECK_IDS.LINK_EXTERNAL_BROKEN),
    });
    ...
    const brokenImageIssues = await this.issues.insertBrokenImageIssues({
      crawlId,
      checkId: CHECK_IDS.IMAGE_SRC_BROKEN,
      severity: severityOf(CHECK_IDS.IMAGE_SRC_BROKEN),
    });
    const redirectChainIssues = await this.issues.insertRedirectChainIssues({
      crawlId,
      checkId: CHECK_IDS.LINK_REDIRECT_CHAIN,
      severity: severityOf(CHECK_IDS.LINK_REDIRECT_CHAIN),
    ...
    const metrics = await this.computeAggregates(
      crawlId,
      crawl.websiteId,
      crawl.sitemapGroupId,
    );
    ...
    await this.state.publish({
      ...this.state.buildProgress(crawlId, CrawlStatus.Completed, counters),
      finishedAt: new Date().toISOString(),
    });
    await this.state.cleanup(crawlId);

    this.logger.log(
      `crawl ${crawlId} finalized: score=${metrics.seoScore}, dupIssues=${duplicateIssues}, ` +
   ...
   */
  private async detectSchemaChanges(
    crawlId: string,
    websiteId: string,
    ...
    for (let i = 0; i < pageIds.length; i += BATCH) {
      ...
      const records: CrawlChangeInput[] = [];
      for (const pageId of batch) {
        ...
        for (const change of diffPageSchema(before, after)) {
          records.push({
            pageId,
      ...
      if (records.length > 0) total += await this.changes.insertMany(crawlId, websiteId, records);
    }
    return total;
  }
  ...
  private async computeAggregates(
    ...
    websiteId: string,
    sitemapGroupId: string | null,
  ): Promise<{ seoScore: number; schemaEntities: number }> {
    ...
    for (const s of severity) bySeverity[s.severity] = s.count;
    ...
    const schemaStatus: Record<string, number> = {};
    for (const s of statusCounts) schemaStatus[s.status] = s.count;
...
function groupByPage(rows: SchemaSummaryRow[]): Map<string, SchemaSummaryRow[]> {
  const map = new Map<string, SchemaSummaryRow[]>();
...
function toEngineSummary(row: SchemaSummaryRow): SchemaEntitySummary {
  return {
    type: row.schemaType,
    format: row.format as SchemaEntitySummary['format'],
    identity: row.identity,
    entityHash: row.entityHash,
    status: row.status as SchemaEntitySummary['status'],
    properties: (row.properties ?? {}) as SchemaEntitySummary['properties'],
    richProfiles: Array.isArray(row.richResults)
      ? (row.richResults as Array<{ profile: string; status: string }>).map((r) => ({
          profile: r.profile,
          status: r.status as SchemaEntitySummary['richProfiles'][number]['status'],
        }))
      : [],
  };
}
